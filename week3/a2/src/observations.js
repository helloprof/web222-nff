...
function transformObservation(original) {
  // TODO
  function isExtinct(original) {
    const conservationStatus =  original.taxon.conservation_status
    const statusName = original.taxon.conservation_status.status_name
    if (conservationStatus) {
      if (statusName !== null) {
        if (statusName == 'extinct in the wild') {
        return true
      } else {
        return false
      }
  ...
  let extinct = isExtinct(original)
  ...
  // let newPhotos = original.photos.map((photo) => {
  //   // {
  //   //   url: photo.url,
  //   //   copyright: photo.attribution
  //   // }
  ...
  // console.log(newTransformedObject)
...
function transformObservations(data) {
  ...
  data.results.forEach((result) => {
    let transformed = transformObservation(result)
    transformedCases.push(transformed)
  })
  // console.log(transformedCases)
